...
public class TicketMasterAPI implements ExternalAPI {
	...
	public List<Item> search(double lat, double lon, String term) {
		...
		String geoHash = GeoHashEncoding.encodeGeohash(lat, lon, 4);
		...
		try {
			...
			// before this line is just create a connection instance but didn't create real
			// connection yet.
			...
			// connection.getInputStream(): Return an inputStream instance that reads
			// response data from this open connection. Then we need to parse the
			// InputStream.
			// Create a BufferedReader instance to help read text from a character-input
			// stream. Provide for the efficient reading of characters, arrays, and lines.
			...
			// Obtain part of the Json object - a Json array that represents events. Where
			// are the "_embedded" and "events" come from?
			// They come from the response structure from TicketMaster, so we know what
			// information are we need
...
//	}
	...
	// Check if there is no key value pair associated with the given key or if the
	// value is the
	// JSONObject.NULL object. Must check before calling getXXX(). Otherwise
